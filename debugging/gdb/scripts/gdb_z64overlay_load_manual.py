# from https://github.com/fig02/gdb-load-ovl
#      https://github.com/fig02/gdb-load-ovl/tree/327147060af067c4e2ff4793b4d46249afeab161

# Once loaded, this script provides a command `ovl`,
# that allows loading into gdb a z64 overlay *that is already loaded in-game*.
# Note: the z64 overlays aren't automaticalled unloaded in gdb when they get unloaded in-game.
...
def AddOverlaySymbols(overlay_table, index):
    ...
    target_func_name = (
        gdb.execute(f"info symbol {vram_address}", False, True)
        .partition(" ")[0]
        .rstrip()
    )
